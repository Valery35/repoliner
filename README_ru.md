...
- **0.1.7** - рядом с `plugins.xml` пишется страница `index.html`, реестр можно смотреть в браузере.

- **0.1.6** - не текстовый файл, выбранный как реестр, даёт сообщение, а не окно ошибки.
- **0.1.5** - репозиторий на английском языке, сообщения об ошибках следуют языку интерфейса QGIS.
